...
export interface PersistentAgentProfile {
	...
	role: string
	/** System prompt (optional - auto-generated from name+role if not provided) */
	systemPrompt?: string
	/** If true, agent is temporary and will be deleted after task completion */
	ephemeral?: boolean
	/** Agent lifecycle */
...
export class PersistentAgentStore {
	...
	create(profile: Omit<PersistentAgentProfile, "createdAt" | "lastActiveAt" | "lifecycle" | "status" | "workspaceFolder">): PersistentAgentProfile {
		...
		const agent: PersistentAgentProfile = {
			...profile,
			systemPrompt: profile.systemPrompt || `You are ${profile.name}, a ${profile.role}.`,
			lifecycle: "persistent",
			status: "active",
			workspaceFolder,
			createdAt: new Date().toISOString(),
